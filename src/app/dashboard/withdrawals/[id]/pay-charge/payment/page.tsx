...
import WithdrawalChargePaymentPanel from "@/components/dashboard/WithdrawalChargePaymentPanel";
import Button from "@/components/ui/Button";
import { fetchDashboardJson } from "@/lib/fetch-json";
import { useI18n } from "@/components/providers/I18nProvider";

export default function WithdrawalChargePaymentPage() {
  ...
  return (
    <DashboardGate isLoading={loading}>
      {error || !data ? (
        ...
        <WithdrawalChargePaymentPanel data={data} onRefresh={() => load(true)} />
      )}
    </DashboardGate>
  );
}
